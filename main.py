...
def menu(stdscr):
    ...
    while lastkey != ord('q'): #ord gives the number code of q
        ...
        if enemycount // 3 >= 2:
            f = open("scores.txt", "a")
            f.write(f"Stage 3 completion time: {time} done by {name} \n")
            f.close()
            if won == False:
                wontime = time
            won = True
            title = f"YOU WON! in {wontime} seconds!"
            stdscr.attron(curses.color_pair(4))
            stdscr.attron(curses.A_BOLD)
            stdscr.addstr(y_global, (WIDTH//2) - len(title)//2,title)
            stdscr.addstr(y_global + 1, (WIDTH//2) - len(title)//2,subtitle)
        ...
        if killcount == len(enemy) and won == False:
            enemycount += 3
        ...
        for i in range(len(enemy)):
            ...
            if enemy[i].ex == cx and enemy[i].ey == cy and enemy[i].death == False and won == True:
                isdead = True
